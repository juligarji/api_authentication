module ApiAuthentication
  class User < ApplicationRecord
    ...
    # Exists validation
    validates :email,:name,:lastname, presence: true,on: :create

    # Length Validation
    validates :username, length: {in: 6..18}, on: :create, allow_blank: true
    validates :email, length: {in: 4..50}, on: :create, allow_blank: true
    validates :name, length: {in: 2..40}, on: :create, allow_blank: true
    validates :lastname, length: {in: 2..40}, on: :create, allow_blank: true
    ...
    # Format validation
    validates :username, format: { with: /\A[a-z0-9\-\_]+\z/i, message: "can only contain letters,numbers and dash" }, allow_blank: true
    validates :email, format: {with: /\A[a-z\@\-\_\.]+\z/i, message: "has an invalid format"},allow_blank: true
    validates :name,:lastname, format: {with: /\A[a-zàáâçéèêëîïiíôóûùüúÿñæœ\' ]+\z/i, message: "can only contain letters and numbers"},allow_blank: true
    
    # Uniqueness
    validates :email,:username,:login_token,:recovery_token,:confirmation_token,on: :create, uniqueness: true, allow_blank: true
